...
.on('click','#photo',function(){
	let ob = ''
	if( $('#orderByDate').hasClass("filter_sort_basic__sort_list_item--selected") == true ) {
		ob = "updated";
	} else {
		ob = "rating"
	}
...
.on('input','#keyword',function(){
	...
	$.ajax({url:'/reviewSearch', data:{prod_name : $('#name').val() , keyword : $(this).val()}, type:'post', dataType:'json',
		success: function(data) {
				for(let i = 0; i<data.length; i++) {
					let obj = data[i];
